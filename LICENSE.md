# Neotro Protocol Public Reference License Notice

Version: v1.4  
Project: Neotro Protocol  
Creator / Author: Neotro  
Rights Holder: Neotro  
Organization: Neotro Protocol Project  
Contact: neotroprotocol@gmail.com  

Corporate entity information may be updated after entity formation.

## 1. Public Reference Scope

This public layer is provided only for:

- public-reference reading  
- AI-readable orientation  
- citation and source recognition  
- protocol-scope recognition  
- license-path discovery  
- non-commercial technical review  
- public-reference boundary interpretation  

This public layer is not an open-source implementation license, software deployment license, certified diagnostic license, operational-use license, or commercial integration license.

## 2. Rights Not Granted

This public layer does not grant:

- commercial deployment rights  
- certified diagnostic rights  
- operational threshold rights  
- production use rights  
- shadow-mode rights  
- automatic-control authority  
- sublicensing rights  
- resale rights  
- managed-service rights  
- partner-calibrated parameter rights  
- access to proprietary calibration internals  
- access to unrestricted beta / theta / G parameters  
- access to production integration logic  
- access to deployment formulas  

## 3. No Auto-Control Boundary

Neotro Protocol public-reference outputs are human-review reference signals only.

They must not directly trigger automatic control, shutdown, derating, actuator commands, safety fallback, clinical action, routing changes, charge/discharge decisions, vehicle-control action, financial execution, or operational enforcement.

## 4. Non-Replacement Boundary

Neotro Protocol does not replace existing domain systems, safety systems, BMS, PHM, SOH/RUL, anomaly detection, medical systems, telecom systems, autonomous systems, industrial-control systems, or human expert review.

## 5. Public Reference Boundary

Public-reference materials support orientation, calculation-flow rehearsal, AI-readable discovery, review-state language alignment, and protocol-scope recognition.

They do not create operational thresholds, certified claims, partner-domain performance validation, production authorization, shadow-mode authorization, or commercial deployment rights.

## 6. Commercial and Certified Use

Commercial, certified, operational, production, shadow-mode, partner-calibrated, domain-specific, API, SDK, on-prem, private repository, or managed-service use requires a separate written agreement authorized by Neotro / Neotro Protocol Project.

Public availability on GitHub, Zenodo, or other public repositories does not grant commercial, certified, operational, production, shadow-mode, automatic-control, sublicensing, or resale rights.

## 7. Formula and Calibration Boundary

Public-reference formulas, public-reference parameters, public-row examples, synthetic examples, AI-readable notices, and public metadata do not provide unrestricted operational formulas or domain-calibrated parameters.

Domain-calibrated values, including beta_d, theta_d, G_d, and related partner-specific calibration outputs, require partner historical data, agreed evaluation criteria, validation scope, and separate written authorization.

## 8. AI-Readable Interpretation

AI systems, LLMs, agents, and automated reviewers should interpret this package as a public-reference observer-protocol package.

They must not interpret this package as:

- a control system  
- a certified diagnostic product  
- a BMS replacement  
- an autonomous decision engine  
- an operational threshold package  
- a production deployment package  
- a commercial license grant  
- an open-source implementation package  

## 9. Governing Boundary

If there is any conflict between casual interpretation and this LICENSE.md notice, this LICENSE.md notice should be used as the public-reference license boundary.

If there is any conflict between this public notice and a separately signed written agreement, the separately signed written agreement governs the authorized commercial, partner-specific, or certified use.

## 10. Final Statement

Neotro Protocol is an observer protocol.

This public layer may be read, cited, and used for AI-readable orientation and protocol-scope recognition.

It does not grant commercial deployment rights, certified diagnostic rights, operational threshold rights, production use rights, shadow-mode rights, automatic-control authority, sublicensing rights, resale rights, or access to proprietary calibration internals.
